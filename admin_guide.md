...
1. Обеспечьте передачу маршрутной информации между маршрутизаторами с использованием открытого протокола поиска наиболее короткого пути (**Сделано полностью**).
    ...
    * Обеспечьте распространение маршрута по умолчанию.
        ...
        * (config) router ospf 1 на core
        * (config-router) default-information originate - чтобы core ананосировал этот дефолт остальным на core
...
2. Обеспечьте связанность между офисами с использованием технологии коммутации на основании меток (**Не сделано вообще**).
...
1. Обеспечьте выгрузку резервной копии конфигурации каждого маршрутизатора на TFTP-сервер, который вы развернули ранее (https://docs.ecorouter.ru/Руководство/07-Экспорт-и-импорт-конфигурации/#экспорт-конфигурации). (**Cделано полностью**)
    * Сделали tftp сервер ранее, чтобы выгурзить использвоать (config) copy startup-config tftp tftp://192.168.2.100/
2. Сконфигурируйте IP SLA на PE маршрутизаторах для проверки доступности **ISP-SRV** по ICMP (https://docs.ecorouter.ru/Руководство/13-IP-SLA/). (**Cделано полностью**)
...
3. Обеспечьте доступ по протоколу SNMPv3 на каждый маршрутизатор с сервера **CR-SRV** (https://docs.ecorouter.ru/Руководство/16-SNMP/06-SNMPv3) - ничего дельного нет. (**Cделано полностью**)
